...
///
///     try BCrypt.hash("test", cost: 4)
///
/// BCrypt uses a random salt each time it creates a hash. To verify hashes, use the `verify(_:matches)` method.
///
///     let hash = try BCrypt.hash("test", cost: 4)
///     try BCrypt.verify("test", created: hash) // true
///
...
public final class BCryptDigest {
    ...
    public func generateSalt(cost: Int, algorithm: Algorithm = ._2b, seed: [UInt8]? = nil) -> String {
